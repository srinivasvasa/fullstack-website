<?php 
if (!isset($_SESSION)) {
	session_start();
}
include('./admininclude/header.php');
include('../dbConnection.php');

if (isset($_SESSION['is_admin_login'])) {
	$adminLogemail =$_SESSION['adminLogemail'] ;
}else{
	echo "<script> location.href='../index.php';</script>";
}
?>
<div class="col-sm-9 mt-5 ">
	<form action="" method="POST" class="d-print-none">
		<div class="form-row">
			<div class="form-group col-md-2" style="margin-bottom: 10px;">
				<label>FROM</label>
				<input type="text" class="form-control" id="startdate" placeholder="startdate">
			</div>
			<div class="form-group col-md-2" style="margin-bottom: 10px;">
				<label>TO</label>
				<input type="text" class="form-control" id="enddate" placeholder= "enddate">
			</div>
			<div class="form-group col-md-2">
				<input type="submit" class="btn btn-secondary" id="searchsubmit" value="Search">
			</div>
		</div>
	</form>
	
	<?php
	if (isset($_REQUEST['searchsubmit'])) {
		$startdate = $_REQUEST['startdate'];
		$enddate =$_REQUEST['enddate'];
		$newstartdate=date("Y-m-d",strtotime($startdate));
		$newenddate=date("Y-m-d",strtotime($enddate));
		echo $newenddate;
		echo $newstartdate;
		$sql = "SELECT * FROM courseorder WHERE order_date BETWEEN '$newstartdate' AND '$newenddate'";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
			echo '
			<p class="bg-dark text-white p-2 mt-4"> Details</p>
				<table class="table">
					<thead>
						<tr>
							<th scope="col">Order ID</th>
							<th scope="col">Course ID</th>
							<th scope="col">Student Email</th>
							<th scope="col">Payment Status</th>
						</tr>
					</thead>
				</table>';
				while ($row = $result->fetch_assoc()) {
					echo '<tr>
					<th scope="row>'.$row["order_id"].'</th>
					<td> '.$row["course_id"].'</td>
					<td> '.$row["stu_email"].'</td>
					<td> '.$row["status"].'</td>
					<td> '.$row["order_date"].'</td>
					<td> '.$row["amount"].'</td>
					</tr>';
				}
				echo '<tr>
				<td><form class="d-print-none"><input class="btn btn-danger"  type="button" onclick="javascript:window.print()"></form></td></tr>
					</tbody>
				</table>';	
		}else {
			echo "<div class= 'alert alert-warning col-sm-6 ml-5 mt-2 role ='alert'>No Records Found !</div>";
		}
	}
	?>
<?php 
include('./admininclude/footer.php'); 
?>